...
def format_v2(list):
    ...
    while i < len(list):
        if i == len(list)-1:
            person_ans = set(list[i])
            group_ans.append(person_ans)
            formatted_answers.append(group_ans)
...
def count_yes_answers(answers):
    ...
    for group_answer in answers:
        ...
        all_yes = len(set.intersection(*group_answer))
        print(all_yes)
        count += all_yes

    return count

formatted_answers = format_v2(answers)
print(count_yes_answers(formatted_answers))