...
def format(list):
    ...
    while i < len(list):
        if i == len(list)-1:
            passport += "".join(" "+list[i])
            passport_strings.append(passport)
...
def convert(strings):
    ...
    for item in strings:
        passport_list.append(item.split(" "))
    return passport_list

def find(passport, targets):
    target_indexes = []
    for target in targets:
        for i,passport_field in enumerate(passport):
            for j,key in enumerate(passport_field):
                if key == target:
                    target_indexes.append(i)
    return target_indexes

def passport_validator(list, strict=True):
    passports = format(list)
    number_of_valid_passports = 0
    essential_fields = ["ecl", "pid", "eyr", "hcl", "iyr", "hgt", "byr"]
    formatted_passports = convert(passports)

    if strict == False:
        for passport in passports:
            if all(field in passport for field in essential_fields):
                number_of_valid_passports += 1
                return number_of_valid_passports

    elif strict == True:
        # Now want to split each element at the colon
        for passport in formatted_passports:
            for i in range(len(passport)):
                passport[i] = passport[i].split(":")
            print(passport)
            passport_field_indexes = find(passport, essential_fields)

            if len(passport_field_indexes) != 7:
                print("missing a field")
                continue

            pass_dic = {
                "eye_colour": passport[passport_field_indexes[0]][1],
                "passport_ID": passport[passport_field_indexes[1]][1],
                "expiration_year": passport[passport_field_indexes[2]][1],
                "hair_colour": passport[passport_field_indexes[3]][1],
                "issue_year": passport[passport_field_indexes[4]][1],
                "height": passport[passport_field_indexes[5]][1],
                "birth_year": passport[passport_field_indexes[6]][1]
                                   }

            if not pass_dic["eye_colour"] in ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"]:
                print("Invalid eye colour.")
                continue

            if not len(pass_dic["passport_ID"]) == 9:
                print("Invalid passport_ID.")
                continue

            if not 2020 <= int(pass_dic["expiration_year"]) <= 2030:
                print("Invalid expiration date.")
                continue

            hair_colour = pass_dic["hair_colour"]
            if not (hair_colour[0] == "#" and hair_colour[1:].isalnum() and len(hair_colour) == 7):
                print("Invalid hair colour.")
                continue

            if not 2010 <= int(pass_dic["issue_year"]) <= 2020:
                print("Invalid issue year.")
                continue
            print(pass_dic["height"])

            height = pass_dic["height"]

            if "cm" in height:
                if not height[:3].isnumeric():
                    print("invalid length of height")
                    continue
                if not 150 <= int(height[:3]) <= 193:
            ...
            if "in" in height:
                ...
                if not 59 <= int(height[:2]) <= 76:
                    print("invalid height")
                    continue
            if not any(["cm" in height, "in" in height]):
                print("No units for height")
                continue

            if not 1920 <= int(pass_dic["birth_year"]) <= 2002:
                print(pass_dic["birth_year"] + "is an invalid birth year.")
                continue
...
string = "#63e2ec"
print("There are "+str(passport_validator(passports, True)) + " valid passports.")
