...
"8-9 v: vvvvvvvvvg"]
# Split at colon
def password_check(input, type = "position check"):
    ...
    for string in input:
        ...
        rules = split_string[0].split(" ")
        checked_char = rules[1]

        if type == "occurrence_check":

            lower_bound = int(rules[0].split("-")[0])
            upper_bound = int(rules[0].split("-")[1])

            if lower_bound <= password.count(checked_char) <= upper_bound:
                correct_passwords += 1

        elif type == "position_check":

            positions = rules[0].split("-")


            if (password[int(positions[0])-1] == checked_char or password[int(positions[1])-1] == checked_char) and password[int(positions[0])-1] != password[int(positions[1])-1]:

                correct_passwords += 1

    return correct_passwords

print(password_check(input, "position_check"))
